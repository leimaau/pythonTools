...
f_oldfist_first = open('result_oldfist_first.txt', 'w', encoding='utf-8')
f_oldfinal_final = open('result_oldfinal_final.txt', 'w', encoding='utf-8')
f_oldtone_tone = open('result_oldtone_tone.txt', 'w', encoding='utf-8')
f_first_oldfist = open('result_first_oldfist.txt', 'w', encoding='utf-8')
f_final_oldfinal = open('result_final_oldfinal.txt', 'w', encoding='utf-8')
f_tone_oldtone = open('result_tone_oldtone.txt', 'w', encoding='utf-8')
...
# 基本函数
def old_new(para, file, old_p, new_p):
...
for i in oldfirstList:
    old_new(i, f_oldfist_first, 'old_first', 'first')

print('从中古声母看方言声母完成，文件为 result_oldfist_first.txt')
...
for i in oldfinalList:
    old_new(i, f_oldfinal_final, 'old_final', 'final')

print('从中古韵母看方言韵母完成，文件为 result_oldfinal_final.txt')


oldtoneList = ['平','上','去','入']

for i in oldtoneList:
    old_new(i, f_oldtone_tone, 'old_tone', 'tone')

print('从中古声调看方言声调完成，文件为 result_oldtone_tone.txt')

print('########################从方言看中古###################################')

for i in firstList:
    old_new(i, f_first_oldfist, 'first', 'old_first')

print('从方言声母看中古声母完成，文件为 result_first_oldfist.txt')


for i in finalList:
    old_new(i, f_final_oldfinal, 'final', 'old_final')

print('从方言韵母看中古韵母完成，文件为 result_final_oldfinal.txt')

for i in toneList:
    old_new(i, f_tone_oldtone, 'tone', 'old_tone')

print('从方言声调看中古声调完成，文件为 result_tone_oldtone.txt')
